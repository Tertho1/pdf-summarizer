...
def process_pdf():
    """Extracts text and summarizes the selected PDF."""
    ...
    try:
        btn_summarize.config(state=tk.DISABLED)  # Disable button while processing
        ...
        summary_text.delete(1.0, tk.END)  # Clear loading text
        summary_text.insert(tk.END, summary)  # Display summary
    except Exception as e:
        messagebox.showerror("Error", f"An error occurred:\n{e}")
    finally:
        btn_summarize.config(state=tk.NORMAL)  # Re-enable button
...
# Scrollable text area for displaying summary
